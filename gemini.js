...
async function generateYouTubeSummary(url, customPrompt) {
  if (!geminiClient) {
  ...
  try {
    ...
    console.log(
      "Successfully fetched YouTube transcript, generating summary..."
    );

    // Use the provided custom prompt or a default one
    const prompt = customPrompt
      ? customPrompt.replace("{transcript}", transcript)
      : `Please provide a comprehensive summary of this YouTube video based on its transcript. Include the main topics discussed and key points *Transcript*:\n\n${transcript}`;
...
async function extractContentUsingScripting(tabId) {
  console.log(
    "Attempting to extract content using scripting API for tab:",
    tabId
  );
  ...
  try {
    ...
    if (results && results[0] && results[0].result) {
      console.log(
        "Successfully extracted content via scripting, length:",
        ...
        chrome.tabs.sendMessage(
          ...
          (response) => {
            ...
            if (response && response.content) {
              console.log(
                "DOM content extracted successfully, length:",
...
async function extractPageContent(tab) {
  ...
  try {
    ...
    console.log(
      "DOM extraction failed or returned insufficient content, trying fallback methods"
    );
